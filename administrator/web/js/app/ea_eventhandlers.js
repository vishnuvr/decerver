...
initRPCEventHandlers = function(){
	...
	eth.RPCEventHandlers["CompileMutan"] = updateCompilerOutput;
...
updateCompilerOutput = function(result){
	if(result.Success){
		var resString = "<samp><b>Compilation successful. Bytes: ";
		var numPerRow = 40;
		var rest = result.Bytes.length % numPerRow;
		var blocks = Math.floor(result.Bytes.length / numPerRow);
		if(blocks > 0){
			for(var i = 0; i < blocks; i++){
				for(var j = 0; j < numPerRow; j += 2){
					resString += result.Bytes.substr(i*numPerRow + j, 2) + " ";
				}
				resString += "\n";
			}
		}
		if(rest > 0){
			for(var j = result.Bytes.length - rest; j < result.Bytes.length; j += 2){
				resString += result.Bytes.substr(j, 2) + " ";
			}
		}
		resString += "</b></samp>"
		$('#CompilerOutput').html(resString);
	} else {
		var resString = "Compilation failed: ";
		for(var i = 0; i < result.Errors.length; i++){
			resString += result.Errors[i] + "\n";
		}
		$('#CompilerOutput').text(resString);
	}
}
...
handleTxPre = function(result){
	// Add this transaction to the transaction window. Its status now is "pending".
...
handleTxPost = function(result){
	...
	if(listEntry === null){
		...
		listEntry.innerHTML = listEntry.innerHTML + "<br>ACCEPTED";
	}
}
...
handleTxPostFail = function(result){
	...
	if(listEntry === null){
		...
		listEntry.innerHTML = listEntry.innerHTML + "<br>REJECTED: " + result.Error;
	}
	var theIdx = -1;
	for(var i = 0; i < myTxs.length; i++){
		var hash = myTxs[i];
...
function getTxPoolLi(txData){
	...
	var li = '<li id=' + id + '><h4>Transaction</h4>Info: ' + txFromTo + '<br>Value: ' + value[0] + ' ' + value[1] + '</li>';
